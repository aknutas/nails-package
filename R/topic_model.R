...
#' @return A list with vector of words (textlist) and data frame with only
#' abstracts, topics and rowids (doctablewt)
#'
#' @export
...
preprocess_literature_for_topicmodeling <- function(literature) {
    ...
    doctablewt <- literature[,c("DocumentTitle",
                                "Abstract",
                                "topicmodelrowids")]
...
build_stm_corpus <- function(textlist) {
    ...
    # Lowercases, removes SMART stopwords, removes numbers, removes punctuation,
    # wordlength min is 3
...
#' @return A list with topic estimation from stm lib (topickest),
#' semantic coherence values (semcohvalues)
#' and estimate of best K of topics (bestk)
#'
#' @export
...
select_optimal_k <- function(data) {
    ...
    # Estimate number of topics; semantic coherence often good
    # (default method spectral; best compromise and deterministic)
    # Seed set for consitent results
    set.seed(5707363)
    topickest <- stm::searchK(out$documents, out$vocab, K = c(4:12),
                              seed = 5707363, verbose = FALSE)
    semcohsK <- data.frame(topickest$results$K, topickest$results$semcoh)
    colnames(semcohsK)<- c("K","semcohs")

    # Getting the K with highest semantic coherence,
    # setting it as true in semcohsK DF
...
topicmodels_json_ldavis <- function(fitted, corpus, doc_term){
    ...
    # elegant, silenced solution
    # from http://stackoverflow.com/a/18749888/5514568
...
build_topicmodels_corpus <- function(textlist) {
    ...
    # Preprocess by lowercasing, removing punctuation, numbers, whitespace
    # and stopwords, and finally stemming
    abstractCorpus <- tm::tm_map(abstractCorpus,
    ...
    abstractCorpus <- tm::tm_map(abstractCorpus,
                                 tm::removeWords, tm::stopwords("SMART"))
...
#' @return List with topicmodels fit (fit), thetaDF distributions (thetadf)
#' and subset of literature columns with topicmodel columns added (doctablewt)
#'
#' @export
...
build_topicmodel_from_literature <- function(literature, K) {
    ...
    abstractDTM <- tm::DocumentTermMatrix(abstractCorpus,
    ...
    if(!is.null(empty.rows)){
        ...
        # Enabling a second pass to prevent a discrepancy between corpus,
        # doclist and DTM
        abstractDTM <- tm::DocumentTermMatrix(abstractCorpus,
                                              control = list(minWordLength = 3))
    }
    ...
    # random starts for model evaluation,
    # best is picked (increase from 1 to 4-6 for final analysis)
    nstart <- 4
    best <- TRUE

    # Latent Dirichlet Allocation
    fit <- topicmodels::LDA(abstractDTM, K, method="Gibbs",
